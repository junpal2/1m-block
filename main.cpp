...
void usage(){
	...
	printf("syntax : 1m-block top-1m.txt\n");
}
...
void readfile(const char* filename){
	...
	time(&end);
	double load_time=difftime(end, start);
	printf("[INFO] Blocklist Load Time : %.6f sec",load_time);
}
...
void dump(unsigned char* buf, int size) {
	...
	if (host) {
		...
		if(host){
			...
			bool exists = blocked.find(host) != blocked.end();
			clock_gettime(CLOCK_MONOTONIC, &t2);
			double search_time = (t2.tv_sec - t1.tv_sec) + (t2.tv_nsec - t1.tv_nsec) / 1e9;
			if (exists) {
				printf("\n>>> Blocked Host matched: %s\n", host);
			} else {
				printf("\n>>> Host mismatch: %s (allowed)\n", host);
			}
			printf("[INFO] Host Search Time: %.6f sec\n", search_time);
			dropflag = exists;
		} else {
